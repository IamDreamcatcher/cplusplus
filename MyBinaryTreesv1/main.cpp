        ...
        class MapIterator : std::iterator<std::bidirectional_iterator_tag, std::pair<KeyType, ValueType>> {
            ...
            public:
                ...
                node* next_node(node* current_node) {
                    if (current_node -> right_ != nullptr) {
                        return find_min_node(current_node -> right_);
                    }
                    node* parent = current_node -> parent_;
                    while (parent != nullptr && current_node == parent -> right_) {
                        current_node = parent;
                ...
                node* prev_node(node* current_node) {
                    if (current_node -> left_ != nullptr) {
                        return find_max_node(current_node -> left_);
                    }
                    node* parent = current_node -> parent_;
                    while (parent != nullptr && current_node == parent -> left_) {
                        current_node = parent;
                        parent = current_node -> parent_;
                    }
                    return parent;
